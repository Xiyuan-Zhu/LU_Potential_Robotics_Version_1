...
class PurePursuit(object):
    ...
    def odom_callback(self, odom_msg):
        global flag
        global VELOCITY
        if flag==0 or (flag==1 and VELOCITY>3.5):
	    global wp_xs
	    global wp_ys
	    global wp_dir
	    global wp_vel
	    ...
	    if flag==0:
                ...
                global Slow_down_param
                VELOCITY=odom_msg.twist.twist.linear.x-Slow_down_param
    ...
    def scan_callback(self, scan_msg):
        ...
        global VELOCITY
        if obj_dect == 1 and flag == 0:
            dis_num=0
            proc_ranges=[x for x in scan_msg.ranges]
            scan_min_idx=int((-0.79-scan_msg.angle_min)/scan_msg.angle_increment)
            scan_max_idx=int((0.79-scan_msg.angle_min)/scan_msg.angle_increment)
            idx=scan_min_idx
            dis_num==0
            objs=0
            while idx < scan_max_idx:
                if proc_ranges[idx]-proc_ranges[idx+1] > dis_threshold:
                    dis_num=1
                    objs=idx
                    idx=idx+10
                if dis_num==1 and proc_ranges[idx+1]-proc_ranges[idx] > dis_threshold:
                    if (idx-objs)*scan_msg.angle_increment*proc_ranges[idx]<4:
                        dis_num=2
                        flag=1
                        #print(proc_ranges)
                idx=idx+1

        if flag==1 and VELOCITY<3.5:
            global circle_r
            global bubble_size
            proc_ranges=[x for x in scan_msg.ranges]
